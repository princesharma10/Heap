/*
   In removing element of the heap :
   1. Swap V[1] and V[last]
   2. Shrink/Remove the last node
   3. Heapify
*/

#include<iostream>
#include<vector>
using namespace std;
class Heap{
    vector<int>v;
    bool minHeap;
    //compare function for min and max heap 
    bool compare(int a ,int b){
        if(minHeap){
            return a<b;
        }
        else{
            return a>b;
        }
    }
    void heapify(int idx){
        int left = 2*idx;
        int right = left +1;
        int min_idx = idx;
        int last = v.size()-1;
        if(left <= last && compare(v[left],v[idx])){
            min_idx = left;
        }
        if(right <= last && compare(v[right],v[min_idx])){
            min_idx = right;
        }
        if(min_idx != idx){
            swap(v[idx],v[min_idx]);
            heapify(min_idx);
        }
    }
public:
     Heap(int default_size = 10, bool type = true){
          v.reserve(default_size);
          v.push_back(-1); //blocking the zeroth index with insertinf -1 character 
          minHeap = type ;
     }
     void push(int d){
         v.push_back(d);
         int idx = v.size()-1; //getting the index of last indec where element is store 
         int parent = idx/2;
         //keep pushing to the top till you reach a root node or stop node or stop midway because condtion is satisfy
         while(idx>1 and compare(v[idx],v[parent])){
             swap(v[idx],v[parent]);
             idx = parent;
             parent = parent/2;
         }
     }
     //make a top function for returning the top most element 
     int top(){
         return v[1]; //returning the top element 
     }
     //make a pop function to delete a element form the heap 
     void pop(){
         int last = v.size()-1;
         swap(v[1],v[last]); 
         v.pop_back();
         heapify(1);
     }
     bool empty(){
         return v.size()==1;
     }


};


int main(){
    Heap h;
    int n;
    cin>>n;
    for(int i=0;i<n;i++){
        int no;
        cin>>no;
        h.push(no);
    }
    //removing all the elements one by one 
    while(!h.empty()){
        cout<<h.top()<<" ";
        h.pop();
    }

    return 0;
}